"""
process_and_upload_semantic.py
================================
Ingests rnf_A.txt using SemanticChunker.
Creates 10 Qdrant collections:
  - rnf_A_semantic_collection_{128,256,512,1024,2048}        -> OpenAI text-embedding-3-large
  - rnf_google_semantic_collection_{128,256,512,1024,2048}   -> Google gemini-embedding-2
"""
import os
import time
...
qdrant_cfg = QDRANT_CONFIGS["semantic"]
...
def get_semantic_chunks(text, size):
    try:
        chunker = SemanticChunker(embedding_model="minishlab/potion-base-8M", chunk_size=size)
...
def main():
    ...
    for size in CHUNK_SIZES:
        chunks = get_semantic_chunks(text, size)
        if ensure_collection(f"rnf_A_semantic_collection_{size}", size_val=3072):
            print(f"  [OpenAI] rnf_A_semantic_collection_{size}")
            upload(f"rnf_A_semantic_collection_{size}", chunks, embed_openai)
        if ensure_collection(f"rnf_google_semantic_collection_{size}", size_val=3072):
            print(f"  [Google] rnf_google_semantic_collection_{size}")
            upload(f"rnf_google_semantic_collection_{size}", chunks, embed_google)
